...
class StandardBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, convolution: Type = nn.Conv2d,
        ...
        # Init pooling
        self.pooling = pooling(kernel_size=(1, 1))
...
class DenseNetBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, convolution: Type = nn.Conv2d,
        ...
        self.first_mapping = nn.Sequential(
            convolution(in_channels=32, out_channels=32, kernel_size=(3, 3), stride=(1, 1),
                        padding=(1, 1), bias=True),
            normalization(num_features=32, affine=True, track_running_stats=True),
            activation()
        )
        # Init second mapping
        self.second_mapping = nn.Sequential(
            convolution(in_channels=64, out_channels=32,
...
class Backbone(nn.Module):
    ...
    def __init__(self, channels: Tuple[Tuple[int, int], ...] = ((64, 64), (64, 64), (64, 64), (64, 64), (64, 64)),
        ...
        self.input_convolution = nn.Sequential(convolution(in_channels=64, out_channels=64,
                                                           kernel_size=(3, 3), stride=(1, 1), padding=(1, 1),
                                                           bias=True),
                                               pooling(kernel_size=(1, 1)))
        # Init blocks
